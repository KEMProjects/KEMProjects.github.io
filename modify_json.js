...
function getJLPTKanji(myList){
	for (var i = 0; i < myList.length; i++) {
		let kanj_str=myList[i]["Kanji"];
		myList[i]["Play Audio"]=true;
		...
		//myList[i]["Audio Clip"]=myList[i]["Audio Clip"]=createAudioPlayer(myList[i]["Vocab"],"");
			...
			if(typeof matches_array !== 'undefined' && matches_array !== null){
				...
				if(jlpt_array.length>=matches_array.length)
					myList[i]["Genki Kanji Level"]=2;
				else if(jlpt_array.length>0)
					myList[i]["Genki Kanji Level"]=1;
				else
					myList[i]["Genki Kanji Level"]=0;
...
function modifyJson(myList){
	...
	getJLPTKanji(myList);
	var myJSON = JSON.stringify(myList, null, '\n');
